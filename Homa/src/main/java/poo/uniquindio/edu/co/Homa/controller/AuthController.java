...
public class AuthController {
    ...
    @Operation(summary = "Iniciar sesión", description = "Autentica un usuario y devuelve un token JWT si las credenciales son correctas.")
    @PostMapping("/login")
    public ResponseEntity<LoginResponse> login(@Valid @RequestBody LoginRequest request) {
        return ResponseEntity.ok(authService.login(request));
    }

    @Operation(summary = "Cerrar sesión", description = "Invalida el token JWT actual (opcionalmente).")
    @PostMapping("/logout")
    ...
    @Operation(summary = "Refrescar token", description = "Genera un nuevo token JWT a partir de un token de refresco válido.")
    @PostMapping("/refresh")
